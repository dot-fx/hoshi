    ...
    <Dialog.Content class="sm:max-w-[425px] bg-background border-border p-0 overflow-hidden sm:rounded-2xl shadow-lg z-[100]">
        ...
        <div class="p-6 pb-4 border-b border-border/40 bg-muted/10">
            ...
            <Dialog.Description class="text-sm font-medium mt-1">
                {#if mode === 'create'}
                    {i18n.t('watchparty.modal.create_room_desc')}
                {:else}
                    {i18n.t('watchparty.modal.join_room_desc')}
                {/if}
            </Dialog.Description>
        </div>
        ...
        {#if mode === 'create'}
            <form onsubmit={handleCreate} class="p-6 space-y-5">
                <div class="space-y-2">
                    <Label for="name" class="font-bold">{i18n.t('watchparty.modal.room_name')}</Label>
                    <Input id="name" bind:value={name} placeholder={i18n.t('watchparty.modal.room_name_placeholder')} class="h-11 rounded-xl bg-muted/10 border-border/50 focus-visible:ring-1 focus-visible:ring-primary/50 font-medium" required disabled={loading} />
                </div>
                <div class="space-y-2">
                    <Label for="password" class="font-bold">{i18n.t('watchparty.modal.password')} <span class="text-muted-foreground font-normal text-xs ml-1">({i18n.t('watchparty.modal.optional')})</span></Label>
                ...
                <div class="flex flex-row items-center justify-between rounded-xl border border-border/50 bg-muted/10 p-4 shadow-sm">
                    <div class="space-y-0.5">
                        <Label class="text-base font-bold cursor-pointer" for="isPublic">{i18n.t('watchparty.modal.public_room')}</Label>
                        <p class="text-xs text-muted-foreground font-medium">{i18n.t('watchparty.modal.public_room_desc')}</p>
                    </div>
        ...
        {:else}
            <form onsubmit={handleJoin} class="p-6 space-y-5">
                <div class="space-y-2">
                    <Label for="joinUrl" class="font-bold">{i18n.t('watchparty.modal.room_url_or_id')}</Label>
                    <Input id="joinUrl" bind:value={joinUrl} placeholder={i18n.t('watchparty.modal.room_url_placeholder')} class="h-11 rounded-xl bg-muted/10 border-border/50 focus-visible:ring-1 focus-visible:ring-primary/50 font-medium" required disabled={loading} />
                </div>
                <div class="pt-4 flex gap-3 w-full">
                    <Button type="button" variant="outline" class="flex-1 h-11 rounded-xl font-bold border-border/50" onclick={() => open = false} disabled={loading}>{i18n.t('watchparty.modal.cancel')}</Button>
