    ...
    $effect(() => {
        if (cid.startsWith("ext:") && !isResolving) {
            ...
            contentApi.resolveExtensionItem(extName, extId)
                .then(async res => { // <-- Añadido 'async' aquí
...
{#if cid.startsWith("ext:") || isResolving}
    <div class="min-h-screen bg-background flex flex-col items-center justify-center gap-4">
        ...
        <h2 class="text-2xl font-bold tracking-tight">Importing Content...</h2>
        <p class="text-muted-foreground">Setting up the entry in your database.</p>
    </div>
{:else}
