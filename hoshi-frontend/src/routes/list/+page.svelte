    ...
    <section class="space-y-6">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-border/40 pb-4 w-full overflow-hidden">
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto shrink-0">
                ...
                <span class="text-sm font-bold text-muted-foreground hidden lg:block">
                    {i18n.t('format_label')}
                </span>
        ...
        {:else if filteredEntries.length === 0}
            ...
            <Empty.Root class="border border-dashed rounded-xl py-16 min-h-[50vh] flex items-center justify-center">
                ...
                <Empty.Header>
                    ...
                    <Empty.Title>
                        {i18n.t('empty_list_title')}
                    </Empty.Title>

                    <Empty.Description>
                        {i18n.t('empty_list_desc')}
                    </Empty.Description>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 3xl:grid-cols-10 gap-x-4 gap-y-10 md:gap-x-5 md:gap-y-12">
                ...
                {#each filteredEntries as entry (entry.cid)}
                    ...
                    <div in:fade={{ duration: 300 }} class="group relative flex flex-col">
                        ...
                        <div class="relative w-full overflow-hidden rounded-xl shadow-lg border border-border/50 group-hover:border-primary/40 transition-all duration-300">
                        ...
                        <div class="mt-4 flex items-start justify-between">
                            ...
                            <div class="flex flex-col min-w-0 pr-2">
                                ...
                                <span class="text-[10px] font-black uppercase tracking-widest text-primary truncate mb-1">
                                    {i18n.t(entry.status.toLowerCase() + '_status')}
                                </span>

                                <span class="text-sm font-black tabular-nums text-foreground/90">
                                    {entry.progress}
                                    <span class="text-muted-foreground/60 font-bold text-xs">
                                        / {entry.totalUnits || '?'}
                                    </span>
                                </span>
                            ...
                            <Button
                                    variant="secondary"
                                    size="icon"
                                    class="h-9 w-9 rounded-lg bg-muted/40 md:opacity-0 group-hover:opacity-100 transition-all shrink-0 shadow-sm hover:bg-primary hover:text-primary-foreground"
