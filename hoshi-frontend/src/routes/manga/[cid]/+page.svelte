<script lang="ts">
    ...
    import MangaHero from "$lib/components/anime/AnimeHero.svelte";
    import MangaSidebar from "$lib/components/anime/AnimeSidebar.svelte";
    ...
    import ChapterTable from "$lib/components/ChapterTable.svelte";
    ...
    const mangaExtensionsPromise = $derived(extensionsApi.getManga());
    ...
    $effect(() => {
        ...
        const updateIndicator = async () => {
            await tick();
            const list = document.getElementById("manga-tabs-list");
            if (list) {
        ...
        <div in:fade={{ duration: 500 }} class="w-full">
            ...
            <div class="w-full px-4 md:px-12 relative z-20 space-y-8 -mt-4 md:-mt-8 max-w-[1400px] mx-auto">
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div class="lg:col-span-4 xl:col-span-3">
                        <MangaSidebar metadata={content} trackers={content.trackerMappings || []} />
                    </div>
                </div>
            </div>
        </div>
